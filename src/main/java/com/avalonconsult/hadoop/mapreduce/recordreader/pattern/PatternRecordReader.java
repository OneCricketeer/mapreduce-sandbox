...
public class PatternRecordReader extends RecordReader<LongWritable, Text> {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(PatternRecordReader.class);
  ...
  private final Text headerMatch = new Text();
  ...
  public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException, InterruptedException {
    ...
    this.maxLineLength = job.getInt(LineRecordReader.MAX_LINE_LENGTH, Integer.MAX_VALUE);
    ...
    if (skipFirstLine) {
      Text tmp = new Text();
      start += in.readLine(tmp, 0, (int) Math.min(Integer.MAX_VALUE, end - start));
    }
  ...
  public boolean nextKeyValue() throws IOException {
    ...
    while (pos < end) {
      ...
      newSize = readNext(value, maxLineLength, Math.max((int) Math.min(Integer.MAX_VALUE, end - pos), maxLineLength));
      ...
      LOG.debug(String.format(fmt, newSize, (pos - newSize)));
    }
